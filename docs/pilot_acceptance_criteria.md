...
## Mindestkriterien für `live erreichbar`

Alle Punkte müssen gelten:

1. `GET /health/live` liefert `200`.
2. Die Produktionsumgebung läuft mit `ENVIRONMENT=production`.
3. Keine Runtime-Schema-Heilung im Live-Standard.
4. Frontend und Backend sind über die produktive Domain erreichbar.

## Mindestkriterien für `ready for pilot`

Alle Punkte müssen gelten:

1. `GET /health/ready` ist nicht `unhealthy`.
2. Der moderne Release-Smoke endet nicht mit `live_failed` oder `business_smoke_failed`.
3. Die offiziellen Regional-Endpunkte liefern für den Pilot-Scope erfolgreich JSON:
...
4. Forecast-Recency für den Pilot-Scope ist nicht kritisch.
...
8. Ein interner Operator kann die Pilot-Ausgaben über das Runbook reproduzierbar erzeugen.

## Mindestkriterien für `fully production-grade`

Alle Punkte müssen gelten:

1. Der gesamte offiziell verkaufte Scope ist grün oder bewusst unsupported.
2. Operative Forecast-Snapshots werden regelmäßig neu geschrieben.
3. Forecast-Recency läuft nicht mehr aus Trainings-Snapshots, sondern aus operativen Snapshots.
4. Kritische Source-Coverage-Probleme sind für den verkauften Scope beseitigt oder bewusst aus dem Support herausgenommen.
...
- Trotzdem bleiben sie ausserhalb der aktiven Produktfreigabe, solange das operative Quality Gate nicht grüner wird.
...
- `SARS-CoV-2` bleibt standardmäßig:
  - `rollout_mode = shadow`
...
5. `SARS-CoV-2` bleibt standardmäßig shadow/watch-only.
...
- alle Kriterien für `ready for pilot` erfüllt sind
...
- Der nächste harte Hebel ist nicht Uptime, sondern die fachliche Freigabe mindestens eines kleinen Day-one-Pilot-Scopes.
