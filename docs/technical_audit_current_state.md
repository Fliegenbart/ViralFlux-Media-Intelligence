...
| `frontend/src/pages/media/NowPage.tsx` | Loads weekly decision, evidence, wave outlook, regional benchmark, regional portfolio | mature | Depends on multiple backend paths at once | Strong fit for current decision workflow |
...
| `frontend/src/components/cockpit/NowWorkspace.tsx` | Main decision UI with GO/WATCH state, regional focus, and evidence summary | mature | UI copy and defaults remain PEIX/GELO-specific | Product framing is not yet PEIX-neutral |
